...
function saveEmployee() {
  const employee = new Employee(Number(document.getElementById("empNo").value), document.getElementById("empName").value, document.getElementById("deptName").value);
  let result = addEmployee(employee);
  result.then(resp => resp.json()).then(data => updateTable(data)).catch(error => console.log(error));
  resetInputs();
}
...
function loadEmployee(empNo) {
  ...
  result.then(resp => resp.json()).then(data => updateInputs(data)).catch(error => console.log(error));
}
